            ...
            @else
                <div class="space-y-4">
                    @foreach($pendingEvaluations as $evaluation)
                        <a href="{{ route('learner.course.show', ['course' => \App\Models\Course::find($evaluation->course_id)?->course_code ?? '#']) }}" 
                           class="flex bg-gray-50 rounded-xl shadow-sm overflow-hidden border hover:shadow-md transition cursor-pointer">
                            <!-- Icon -->
                            ...
                            <div class="flex-1 p-4 flex flex-col justify-between">
                                <div>
                                    <p class="text-sm font-semibold">{{ $evaluation->title ?? 'Course Evaluation' }}</p>
                                    <p class="text-xs text-gray-500 mt-1">Course: {{ $evaluation->course_name ?? 'N/A' }}</p>
                                    <p class="text-xs text-gray-500 mt-1">Due: {{ $evaluation->due_date ? \Carbon\Carbon::parse($evaluation->due_date)->format('M d, Y') : 'N/A' }}</p>
                                </div>
                           ...
                           class="flex bg-gray-50 rounded-xl shadow-sm overflow-hidden border hover:shadow-md transition cursor-pointer">
                            ...
                            <div class="flex-1 p-4 flex flex-col justify-between">
                                <div>
                                    <p class="text-sm font-semibold">{{ $evaluation->title ?? 'Course Evaluation' }}</p>
                                    <p class="text-xs text-gray-500 mt-1">Course: {{ $evaluation->course_name ?? 'N/A' }}</p>
                                    <p class="text-xs text-gray-500 mt-1">Completed: {{ $evaluation->completed_at ? \Carbon\Carbon::parse($evaluation->completed_at)->format('M d, Y') : 'N/A' }}</p>
                                </div>
                            </div>
                        </a>
                    @endforeach
