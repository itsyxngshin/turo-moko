    ...
    <main class="flex-1 px-4 md:px-5 py-3 overflow-y-auto">
        ...
        <section class="bg-white rounded-3xl p-6 md:p-8 mb-8 shadow-sm border border-gray-100 relative">
            ...
            <div class="absolute top-4 right-4 md:top-6 md:right-6" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 z-20 overflow-hidden" style="display: none;">
            ...
            <div class="flex flex-col md:flex-row items-center gap-6 md:gap-8">
                ...
                <div class="flex flex-col gap-1 text-center md:text-left">
                    <h2 class="text-2xl md:text-3xl font-bold text-gray-800">{{ $user->profile->first_name }} {{ $user->profile->last_name }}</h2>
                ...
                <div class="space-y-6">
                    @forelse($user->profile->portfolioSets as $set)
                        ...
                        <div class="flex gap-4 relative">
                            @if(!$loop->last) <div class="absolute left-[19px] top-8 bottom-[-24px] w-0.5 bg-gray-100"></div> @endif
                            <div class="w-10 h-10 rounded-full bg-green-50 border border-green-100 flex items-center justify-center shrink-0 z-10">
                                <span class="text-xs font-bold text-green-600">{{ substr($set->workPortfolio->designation, 0, 1) }}</span>
                            </div>
                            <div>
                ...
                <div class="flex flex-col gap-3">
                    @forelse($user->engagements as $engagement)
                        <div class="p-3 rounded-xl bg-gray-50 border border-gray-100">
                            <h5 class="font-bold text-gray-700 text-sm">{{ $engagement->title }}</h5>
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2 mt-1 text-[10px] text-gray-400">
                                    ...
                                    @if($course->start_date && $course->end_date)
                                        {{ \Carbon\Carbon::parse($course->start_date)->format('M d') }} - 
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" 
        ...
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto relative animate-in fade-in zoom-in duration-200 flex flex-col">
            ...
            <div class="p-6 md:p-8 space-y-8 overflow-y-auto">
                {{-- ... Your existing modal inputs for Name, Photo, Experience, etc ... --}}
            ...
            <div class="sticky bottom-0 bg-white px-6 md:px-8 py-5 border-t border-gray-100 flex justify-end gap-3 z-10 rounded-b-3xl">
                ...
                <button wire:click="saveProfile" class="px-6 py-2.5 rounded-xl text-sm font-bold text-white bg-black hover:bg-gray-800 transition shadow-lg shadow-gray-200">Save Changes</button>
            </div>
        </div>
    </div>
    @endif
