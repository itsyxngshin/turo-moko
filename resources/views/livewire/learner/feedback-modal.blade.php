...
x-on:feedback-submitted.window="handleFeedbackSubmitted()">
        ...
        <button @click="$wire.closeModal(); reset()" 
                class="absolute top-3 right-3 text-gray-400 hover:text-gray-600 transition text-xl leading-none">
            &times;
        </button>
                            ...
                            <template x-for="star in 5" :key="'course-step{{ $step }}-' + star">
                                <button type="button" 
                                    ...
                                    <svg viewBox="0 0 24 24" class="w-9 h-9"
                                         :class="star <= courseRatings[{{ $step }}] ? 'text-yellow-400 fill-current scale-110' : 'text-gray-300 fill-current'">
                        ...
                        <div class="flex justify-center gap-3 mt-4">
                            <template x-for="star in 5" :key="'implementor-step{{ $step }}-' + star">
                                <button type="button" 
                                        ...
                                        class="transition-transform hover:scale-110">
                                    <svg viewBox="0 0 24 24" class="w-9 h-9"
                                         :class="star <= implementorRatings[{{ $step }}] ? 'text-yellow-400 fill-current scale-110' : 'text-gray-300 fill-current'">
                                        <path d="M12 17.27 18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>
                                    </svg>
                                </button>
                            </template>
