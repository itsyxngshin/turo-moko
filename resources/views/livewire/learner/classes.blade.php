...
<div class="relative h-60 rounded-2xl overflow-hidden shadow">
    <img src="{{ $featuredCourse->background ?? '/images/banner.jpg' }}" 
         alt="{{ $featuredCourse->name ?? 'Course Banner' }}" 
...
<section class="bg-white rounded-2xl shadow-md p-6 mt-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-md flex overflow-hidden border h-52"> <!-- Added height -->
            <!-- Course Image -->
            <div class="w-1/2">
            ...
            <div class="p-4 flex flex-col justify-between w-1/2 h-full">
                <div>
                    ...
                    <p class="text-sm text-gray-500">{{ $course->subject ?? 'Subject here' }}</p>
                    <h3 class="text-lg font-bold">{{ $course->course_title }}</h3>
                    <p class="text-xs text-gray-400 mt-1 line-clamp-3">{{ $course->description ?? 'No description available.' }}</p>
                </div>
                <div class="flex justify-end mt-2">
                    <button class="bg-black text-white px-4 py-1.5 rounded-full hover:bg-gray-800">
                        Continue
                    </button>
                </div>
            </div>
