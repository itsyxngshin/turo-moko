<div class="flex h-screen bg-gray-50 font-sans">
    <main class="flex-1 px-4 md:px-5 py-3 overflow-y-auto">
        ...
        <section class="bg-white rounded-3xl p-6 md:p-8 mb-8 shadow-sm border border-gray-100 relative">
            ...
            <div class="absolute top-4 right-4 md:top-6 md:right-6" x-data="{ open: false }">
                ...
                <div x-show="open" 
                    ...
                    style="display: none;">
                    <button wire:click="openEditModal" @click="open = false" class="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-blue-600 flex items-center gap-2">
            ...
            <div class="flex flex-col md:flex-row items-center gap-6 md:gap-8">
                ...
                <div class="flex flex-col gap-1 text-center md:text-left">
                    ...
                    <p class="text-gray-500 font-medium">
                        {{ $active_engagement->title ?? $user->role->role_name }}
                    </p>
                ...
                @if($user->profile->portfolioSets->count() > 0)
                    @foreach($user->profile->portfolioSets as $set)
                        ...
                        <div class="flex gap-4 relative">
                            ...
                            <div class="w-10 h-10 rounded-full bg-blue-50 border border-blue-100 flex items-center justify-center shrink-0 z-10">
        ...
        <section class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 md:p-8 mb-10">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                @forelse ($activeCourses as $course)
                    <div class="bg-white rounded-2xl shadow-md flex flex-col sm:flex-row overflow-hidden border border-gray-100 h-auto sm:h-52 hover:shadow-lg transition">
                        ...
                        <div class="w-full sm:w-1/2 h-48 sm:h-full relative bg-gray-200">
                            ...
                            @endif
                            @if($course->category)
                                <div class="absolute bottom-3 left-3 px-2 py-1 bg-black/60 backdrop-blur-sm rounded text-[10px] font-bold text-white uppercase tracking-wide">
                        ...
                        <div class="p-4 sm:p-5 flex flex-col justify-between w-full sm:w-1/2 h-full">
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mt-2 line-clamp-3">{{ $course->background }}</p>
                            </div>
                            
                            <div class="flex justify-end mt-3">
                                <button class="bg-black text-white px-5 py-2 rounded-full hover:bg-gray-800 text-xs font-bold shadow-md transition transform hover:-translate-y-0.5">
                                    Manage
                                </button>
                            </div>
                        </div>
                ...
                @empty
                    <div class="col-span-full text-center py-12 border-2 border-dashed border-gray-100 rounded-2xl bg-gray-50">
                        <p class="text-lg font-semibold">No courses found 📂</p>
                        <p class="text-xs mt-1">Create a course to see it here.</p>
                    </div>
                @endforelse
            </div>
        </section>
    ...
    @if($showEditModal)
    @teleport('body') {{-- TELEPORT FIX --}}
        <div class="fixed inset-0 z-[999] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" 
